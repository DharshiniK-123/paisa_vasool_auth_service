...
async def seed_admin(db: AsyncSession = Depends(get_db)):
    try:
        ...
        admin = User(
            ...
            password=get_password_hashed("Dharshini@#123"),
            role="admin",
            is_active="active"
        )
        ...
        await db.rollback()
        traceback.print_exc()
        raise HTTPException(status_code=500, detail="Failed to seed admin.")
...
async def toggle_user_status_route(user_id: int,admin=Depends(get_current_admin),db: AsyncSession = Depends(get_db),):
